        ...
        while True:
            ...
            if packet['type'] == 'interest':         # Interest packet received
                ...
                if Interest.Time_out(packet,self.result_save,self.threadLock) == True:
                    if self.interest_queue.qsize() < self.queue_size:     #   self.interest_queue.full() == False:
                        self.interest_queue.put(packet)
                    else:
                        ### FIFO
                        
                        self.interest_queue.get(packet)
                        self.interest_queue.put(packet)
                        ...
                        x = {}
                        i = 0
                        for e in list(self.interest_queue.queue):
                            if not e['content_name'] in x:
                                x[e['content_name']] = [i, e]
                            else:
                                x[e['content_name']][0] = i
                            i += 1
                        x = dict(sorted(x.items(), key=lambda item: item[1][0]))
                        for key, value in x.items():
                            self.interest_queue.queue.remove(value[1])
                            break
                        self.interest_queue.put(packet)
                        '''
                        ### LFU
                        ...
                        for e in list(self.interest_queue.queue):
                            if not e['content_name'] in x:
                                x[e['content_name']] = [0, e]
                            else:
                                x[e['content_name']][0] += 1
                        x = dict(sorted(x.items(), key=lambda item: item[1][0]))
                        for key, value in x.items():
                            self.interest_queue.queue.remove(value[1])
                            break
                ...
                if self.data_queue.qsize() < self.queue_size:    #self.data_queue.full() == False:
                    ...
                    x = {}
                    i = 0
                    for e in list(self.data_queue.queue):
                        if not e['content_name'] in x:
                            x[e['content_name']] = [i, e]
                        else:
                            x[e['content_name']][0] = i
                    ...
                    x = dict(sorted(x.items(), key=lambda item: item[1][0]))
                    for key, value in x.items():
                        self.data_queue.queue.remove(value[1])
                        break
                    self.data_queue.put(packet)
                    '''
                    ### LFU
